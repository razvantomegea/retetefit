...
import { assertReleaseReady, getLastTagVersion, readPkgVersion, sh } from './lib/version.mjs';

const REPO = 'razvantomegea/retetefit';
...
const bullets = rawLog
  ...
  .map((line) => line.replace(/\(#(\d+)\)/, `([#$1](https://github.com/${REPO}/pull/$1))`))
  .map((line) => `- ${line}`)
  .join('\n');
...
try {
  ...
  if (tagPushed) {
    try {
      ...
      console.error(`Failed to delete remote tag ${tag}:`, cleanupError.message ?? cleanupError);
    }
  }
  if (mainPushed) {
    try {
      ...
      console.error('Failed to revert main push:', cleanupError.message ?? cleanupError);
    }
  }
  try {
    sh(`git tag -d ${tag}`);
